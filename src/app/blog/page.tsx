...
export default async function BlogPage() {
  ...
  return (
    <div>
      ...
      <div className='flex flex-row align-middle items-center left-0 ml-8 mt-2 md:mt-4'>
      ...
      <main className='flex flex-col relative py-2 -mt-4'>
        <div className='flex mt-8 mb-16 gap-8'>
          <ul className='grid grid-cols-4 gap-4 px-6 py-4 justify-center'>
            {posts?.map((post) => (
              <li key={post.id} className='px-4 py-2 ml-10 mr-4 border-2 border-red-700 rounded-xl bg-red-600/40 hover:bg-red-600/30 hover:scale-105 transition-all duration-200 shadow-xl shadow-red-700/60 max-h-auto'>
                <Link href={`/blog/${post.slug}`}>
                  <h2 className='text-2xl font-black'>{post.data.title}</h2>
                  <p className='text-lg font-normal max-h-36 overflow-hidden'>{post.data.content}</p>
